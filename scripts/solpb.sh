...
do
  ...
  protoc -I$(pwd)/proto -I "third_party/proto" -I${SOLPB_DIR}/protobuf-solidity/src/protoc/include \
    ...
    --"sol_out=gen_runtime=@0fatih/yui-ibc-solidity/contracts/proto/ProtoBufRuntime.sol&solc_version=0.8.9&ignore_protos=gogoproto/gogo.proto:$GEN_DIR" \
    $(pwd)/$file
done
cp -r "$GEN_DIR/@0fatih/yui-ibc-solidity/contracts" "$(pwd)"
rm -rf "$GEN_DIR"
